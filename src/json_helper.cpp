...
namespace json_helper {
  ...
  Jsonhelper::Jsonhelper(json jsn){
    j=jsn;
  }

  json Jsonhelper::parse(string path){
    ifstream i(path);
    j=json::parse(i);
    i.close();
  ...
  void Jsonhelper::save(string path){
    ofstream o(path, std::ios::out | std::ios::trunc);
    o  << std::setw(4)<<j << endl;// print pretty-> readable
    o.close();
  }

  bool Jsonhelper::is_empty(json jsonObj){
    return jsonObj.empty();
  }

  void Jsonhelper::get_dimension(json jsonObj, int& dim){
    ...
    if(jsonObj.is_primitive()){
      ...
      dim=0;
      while(!jsonObj.empty()&&!jsonObj.is_primitive()){//jsonObj.size()>1){
	try
	  {
	...
	if(sze.size()>2) std::cerr <<"json2mat: json obj "<< key<<"  must be <=2 but is "<< sze.size() << " dimensional!\n" << endl;
	  ...
	  for (unsigned int itr = 0; itr < sze.at(0);itr++) {
	    for (unsigned int itc = 0; itc < sze.at(1);itc++) {
	      if(sze.at(0)==1&&sze.at(1)==1) m(itr,itc) = j[key];
	      if(sze.at(0)>1&&sze.at(1)==1) m(itr,itc) = j[key].at(itr);
	      if(sze.at(0)>1&&sze.at(1)>1) m(itr,itc) = j[key].at(itr).at(itc);
	    }
	  }
	}
      }else m.zeros(0,0);
  ...
  template<typename T>
  void Jsonhelper::json2mat(Mat<T>& m){
    if(j.is_object()){
      if(!this->is_empty(j)){
	// find size
	vector<unsigned int> sze;
	...
	if(sze.size()>2) std::cerr <<"json2mat: json obj "<<j.dump()<<"  must be <=2 but is "<< sze.size() << " dimensional!\n" << endl;
	  ...
	  for (unsigned int itr = 0; itr < sze.at(0);itr++) {
	    for (unsigned int itc = 0; itc < sze.at(1);itc++) {
	      if(sze.at(0)==1&&sze.at(1)==1) m(itr,itc) = j;
	      if(sze.at(0)>1&&sze.at(1)==1) m(itr,itc) = j.at(itr);
	      if(sze.at(0)>1&&sze.at(1)>1) m(itr,itc) = j.at(itr).at(itc);
	    }
	  }
	}
      }else m.zeros(0,0);
  ...
  /*---*/
  template<typename T>
  void Jsonhelper::json2vector3D(string key,vector<vector<vector<T>>>& v){
    v.clear();
    if(j.is_object()){
      // find size
      vector<unsigned int> sze;
